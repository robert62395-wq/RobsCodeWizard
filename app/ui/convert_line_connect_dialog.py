"""Tools > Convert Line Connect Codes dialog.

Phase 2: Numeric -> Alphabetic only (Civil3D compatibility).
Phase 5 (v0.4.5): Alphabetic -> Numeric for OpenRoads export.
"""
...
class ConvertLineConnectDialog(QDialog):
    """Preview + apply numeric->alphabetic line-connect conversion."""

    def __init__(self, rows: List[Dict], selected_rows: List[Dict] | None = None, parent=None):
    ...
    def _build_ui(self) -> None:
        ...
        self.dir_alpha_to_num.setEnabled(False)
        self.dir_alpha_to_num.setToolTip("Available in v0.4.5")
    ...
    def _refresh_preview(self) -> None:
        ...
        preview = preview_changes(rows, limit=20)
        total_changed = sum(1 for r in rows if convert_numeric_to_alpha(r.get("description", ""))[1] > 0)
    ...
    def apply_to_rows(self, rows: List[Dict]) -> List[Dict]:
        """Apply the conversion to the given rows in-place and return them."""
        for row in rows:
            new_desc, count = convert_numeric_to_alpha(row.get("description", ""))
            if count > 0:
