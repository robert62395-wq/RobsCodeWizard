"""Reseed translation_map.json (v0.5.1.2 - fixed matching algorithm).

KEY FIX: code-equality match returns score 100 immediately.
No more hard type filter that prevented almost all matches.
"""
...
def _score(v, o):
    """v0.5.1.2: code-equality first (returns 100), then description similarity. No hard type filter."""
    v_code = v["code"].strip().upper()
    o_code = o["code"].strip().upper()
    if v_code == o_code:
        return 100
    desc_score = fuzz.token_set_ratio(v["description"], o["description"])
    type_bonus = 0
    if v["type"] and o["type"]:
        if v["type"].lower() == o["type"].lower():
            type_bonus = 10
    return min(100, desc_score + type_bonus)
...
def _build_entry(v, o, score):
    ...
    return {
        ...
        "vdt": v, "odot": o,
        "confidence": conf,
        "match_basis": ["code"] if (o and v["code"].upper() == o["code"].upper()) else (["description", "type"] if o else []),
...
def seed(force=False):
    ...
    matched_odot = set()
    for v in vdt_rows:
        match, score = _best_match(v, odot_rows)
