...
title Rob's Code Wizard v0.3.9.5.0 (hotfix .10)

:menu
cls
echo ============================================================
echo   Rob's Code Wizard  -  v0.3.9.5.0  (hotfix .10)
...
python -m pytest -q
echo (pytest exit code: %errorlevel% - continuing regardless)
...
echo.
echo --- [2] Build EXE ---
echo (note: missing folders are skipped automatically)
if not exist ".venv\Scripts\python.exe" (
    echo .venv not found - run option [1] first to bootstrap.
    pause
    goto menu
)
call .venv\Scripts\activate.bat
...
if exist dist\RobsCodeWizard.exe (
    ...
    echo ERROR: expected dist\RobsCodeWizard.exe not found.
)
pause
goto menu
...
if not exist dist\RobsCodeWizard.exe (
    echo dist\RobsCodeWizard.exe not found - building EXE first...
...
call :find_iscc
if "!ISCC_EXE!"=="" (
    echo ERROR: Inno Setup compiler ^(ISCC.exe^) not found in any standard location.
...
if exist dist\RobsCodeWizard_Setup.exe (
    ...
    echo WARNING: expected dist\RobsCodeWizard_Setup.exe not found.
)
pause
goto menu
...
if exist "app\assets\icon.ico" (
    ...
    echo Skipping: app\assets\icon.ico  ^(not found^)
)
...
if exist "app\assets\" (
    ...
    echo Skipping: app\assets\        ^(not found^)
)
...
if exist "app\data\" (
    ...
    echo Skipping: app\data\          ^(not found^)
)
...
if exist "resources\" (
    ...
    echo Skipping: resources\         ^(not found^) ^<-- VDT/ODOT code sheets live here!
)
...
call :find_iscc
if "!ISCC_EXE!"=="" (
    echo ERROR: ISCC.exe not found in any standard location.
    exit /b 1
)
echo Using compiler: !ISCC_EXE!
"!ISCC_EXE!" build\installer.iss
